<?php $page='sold'; include("header.php");?>
	...
	<div class="modal fade" id="insert" role="dialog" aria-labelledby="insert" aria-hidden="true">
        ...
        <tbody>
            ...
            while ($vehicle = mysql_fetch_assoc($res)) { ?>
                ...
                <td><?php echo "ID:",$vehicle['u_id'],"<br> ",$vehicle['f_name']," ",$vehicle['l_name']; ?></td>
                <td width=50px>
	
                    <a href="Actions.php?action=v_edit&id=<?php echo $vehicle['v_id']; ?>" class="btn btn-warning" style="display:none" >Edit</a>
                    <a data-toggle="modal" data-target="#myModal" href="Actions.php?action=vs_view&id=<?php echo $vehicle['v_id']; ?>" class="btn btn-info btn-sm" >View</a>
                    <a onclick="return confirm('All records will be deleted & Vehicle will be unsold!!, continue?')" href="Actions.php?action=vs_delete&id=<?php echo $vehicle['v_id']; ?>" class="btn btn-danger btn-sm" <?php if($userRow['u_type']!="Admin"){echo 'style="display:none"';} ?>  >Delete</a>
